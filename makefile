...
goose:
	...
	go tool goose -dir ./db/migrations postgres "user=postgres password=postgres host=localhost port=5432 dbname=fragtape sslmode=disable" $$action
...
migrate:
	...
	@docker compose up db -d
	@docker compose exec db bash -c 'until pg_isready -U postgres; do sleep 1; done'
	@go tool goose -dir ./db/migrations postgres "user=postgres password=postgres host=localhost port=5432 dbname=fragtape sslmode=disable" up
	@docker compose down db
