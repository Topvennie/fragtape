...
func New(service service.Service, pool *pgxpool.Pool) *Server {
	...
	goth_fiber.SessionStore = session.New(session.Config{
		KeyLookup:      fmt.Sprintf("cookie:%s_session_id", config.GetDefaultString("app.name", "fragtape")),
		CookieHTTPOnly: true,
	...
	port := config.GetDefaultInt("server.api.port", 8000)
	host := config.GetDefaultString("server.api.host", "0.0.0.0")
